...
- 当前阶段：MVP 第一版开发与验收已经完成，后续进入代码理解、维护与功能增强阶段。
- 最小可用版本（MVP）已完成，认证、课程管理和任务管理核心业务流程均可用。
- 后端 MVP 共 15 个接口已完成，并通过编译、自动化测试和真实业务冒烟测试。
- Vue 3 + Vite 前端已完成浏览器 MVP 全流程验收。
...
- 退出登录已增加二次确认，只有确认后才发送请求；提交中和失败状态均有明确反馈。
...
- 课程卡片已通过 `RouterLink` 跳转到动态路由 `/courses/:courseId`，并具有 `hover`、`focus-visible` 和 `active` 交互反馈。
- 详情页通过 `route.params.courseId` 获取课程 ID，并调用 `GET /api/courses/{courseId}`。
- 页面展示课程名称、颜色、`todoCount` 和 `doneCount`，支持加载、错误和正常详情状态。
- 页面提供返回课程列表的链接；Session 失效时跳转登录页，课程不存在时展示后端错误信息。
- 浏览器 Network 真实联调已通过，课程详情请求返回 `200 OK`。

### 课程管理

- 首页已完成新建课程弹窗，支持系统自动分配颜色或选择自定义颜色；创建成功后重新查询课程列表。
- 课程详情页已完成编辑课程弹窗，可修改课程名称和颜色；修改成功后重新查询课程详情和当前任务列表。
- 课程详情页已完成删除课程二次确认，明确提示关联任务会被永久删除；删除成功后返回首页。
- 创建、编辑和删除均处理提交中、后端业务错误与 `401` 登录跳转。
- 课程删除已通过浏览器 Network 真实联调，页面跳转和首页课程列表结果正常。

### 课程详情任务管理

- 已完成 TODO / DONE 任务列表查询，支持加载、错误、空列表和正常列表状态。
- 已完成新建任务、任务展开、快速切换状态、完整编辑和二次确认删除。
- 编辑任务支持修改任务名称、所属课程、截止日期和状态。
- 写操作成功后统一重新查询课程统计和当前任务列表，不手工修改统计数量。
- 各操作均处理提交中、后端错误与 `401` 登录跳转。
- 任务管理完整浏览器 Network 联调已通过。

### MVP 验收与交付

- 浏览器完整业务回归已经通过，认证、课程管理、任务管理和退出登录可以组成完整使用流程。
- 仓库根目录 `README.md` 已完成，包含项目介绍、运行步骤、文档入口和项目状态。
- `assets/screenshots/` 已加入课程首页、课程详情与任务管理的桌面端和窄屏截图。
- `docs/database/sql/init.sql` 已独立提供，可创建 `yuki_todo` 及三张核心数据表。

## 正在进行

- MVP 第一版已经开发完成，不再扩展当前版本的核心功能范围。
- 项目重心转入代码理解、日常维护、测试补充和后续增强功能评估。

## 本次更新

- 确认最小可用版本（MVP）第一版开发与浏览器完整业务验收均已完成。
- 记录退出登录二次确认、README、项目截图和独立数据库初始化 SQL 已完成。
- 将后续阶段调整为代码理解、维护与功能增强。
...
- 完成注册、登录、Session 验证和退出登录认证闭环。
- 完成课程查询、创建、编辑和删除闭环。
- 完成任务查询、创建、展开、编辑、删除和状态切换闭环。
- 完成退出登录二次确认及失败提示。
- 完成 README、项目截图和独立数据库初始化 SQL。
- 完成浏览器 MVP 全流程验收。
...
- 课程详情页通过 `GET /api/courses/{courseId}/tasks` 按 TODO / DONE 查询任务。
- 新建任务使用 `POST /api/tasks`，编辑任务使用 `PUT /api/tasks/{taskId}`。
- 快速切换状态使用 `PATCH /api/tasks/{taskId}/status`，删除任务使用 `DELETE /api/tasks/{taskId}`。
- 任务创建、状态切换、编辑和删除成功后均重新查询后端真实数据。

### 前端课程管理

- 首页使用 `POST /api/courses` 创建课程，并在成功后重新加载课程列表。
- 课程详情页使用 `PUT /api/courses/{courseId}` 完整修改课程名称和颜色。
- 课程详情页使用 `DELETE /api/courses/{courseId}` 删除课程，正确处理 `204 No Content` 并返回首页。
- 删除确认弹窗会显示课程名称，并提示课程下全部任务会被永久删除且无法恢复。

## 修改/新增文件

- MVP 收尾与交付涉及：
  - `README.md`
  - `assets/screenshots/home01.png`
  - `assets/screenshots/home02.png`
  - `assets/screenshots/course-detail01.png`
  - `assets/screenshots/course-detail02.png`
  - `docs/database/sql/init.sql`
...
- 网页预览地址：`http://localhost:5173/`；后端接口由 Vite `/api` 代理到 `http://localhost:8080`。
- 使用浏览器 Network 面板联调 `/api/auth/me`、`/api/courses` 和退出登录请求。
- 使用浏览器 Network 面板联调 `GET /api/courses/{courseId}`。
- 使用浏览器 Network 面板联调课程创建、编辑和删除请求，并核对成功后的页面数据与跳转结果。
- 使用浏览器 Network 面板完成任务列表、新建、状态切换、编辑和删除的完整联调。
- 使用浏览器完成“注册 / 登录 → 创建课程 → 创建任务 → 查询与修改任务 → 切换状态 → 删除任务 → 删除课程 → 退出登录”的 MVP 全流程回归。
...
- 用户已使用 Apifox 完成注册、登录、课程和任务操作的完整业务冒烟测试。
- 本轮状态同步未重新执行上述测试。
...
- Maven 编译：通过，结果为 `BUILD SUCCESS`。
- MockMvc 任务模块：23 个测试通过，0 失败，0 错误。
- 前端 ESLint：通过，0 错误。
- Vite 生产构建：通过，Vite 8.2.0 最新构建共转换 33 个模块。
- Apifox 完整业务冒烟测试：通过。
- 浏览器 MVP 全流程验收：通过。
- 前端课程详情页任务管理各阶段 ESLint 和生产构建均已通过。
- 前端课程创建、编辑和删除各阶段 ESLint 和 Vite 生产构建均已通过。
- 浏览器 Network 联调：删除课程返回 `204 No Content`，随后正确返回首页，课程列表结果正常。
- 浏览器 Network 联调：任务列表、新建、状态切换、编辑和删除流程均已通过。
- 浏览器 Network 联调：认证和课程列表请求成功，Session Cookie 正常携带。
- 浏览器 Network 联调：课程详情请求返回 `200 OK`，页面正确展示课程名称、颜色、`todoCount` 和 `doneCount`。
...
- Apifox 完整业务冒烟测试已通过：
  `注册 / 登录 → 创建课程 → 创建任务 → 查询任务 → 修改任务 → 切换状态 → 删除任务 → 删除课程`。
- 本轮状态同步没有重新执行 Maven、MockMvc、真实 HTTP 或 Apifox 测试。
...
- `CourseDetailView.vue` 集中了较多页面状态和事件函数，初学者阅读成本较高，后续需要在充分理解数据流后逐步整理。
...
1. 补充认证模块和课程模块的自动化测试。
2. 提升代码可读性，并按照页面事件、业务函数和 API 请求逐步理解前端数据流。
3. 在 MVP 稳定维护的基础上，评估日历、统计和子任务等增强功能。
